...
private:
	...
	PageDescriptor *split_block(PageDescriptor **block_pointer, int source_order)
	{
		...
		// remove the block starting at the current order
		// and insert two blocks into the order below

		// find the buddy
		
		// This is variable to store the pointer to the buddy to the pointer.
		PageDescriptor *buddy_pointer;

		// The pointer to the buddy is the pointer to the block pointer 
		// plus a half size of the block.
		buddy_pointer = *block_pointer+(pages_per_block(source_order)/2);
		...
		insert_block(original_pointer,source_order-1);
...
public:
	...
	PageDescriptor *alloc_pages(int order) override
	{
		...
		// return NULL because there is no free pages to be allocated.
		// After this while loop finishes, the ord will be the lowest order 
		...
		// dlot is a pointer to a pointer
		PageDescriptor **slot; 
	...
	void free_pages(PageDescriptor *pgd, int order) override
	{
		...
		while(count<MAX_ORDER-1){
			...
			// If there are no available block on this order.
			...
			// If PGD buddy is found on this order.
	...
	bool reserve_page(PageDescriptor *pgd){
		...
		for(int current_order = 0;current_order<=MAX_ORDER;current_order++){
			...
			// while slot is not null, if pgd is within this block,
			...
			// if a slot is null, this means the the current order has no available
			// blocks, start next iteration to search for next order.
			if(slot == NULL)
				continue;
			
			// If the block contains the slot is found,
			// continuously split the block until order 0 
			else{
	...
	bool init(PageDescriptor *page_descriptors, uint64_t nr_page_descriptors) override
	{
		...
		for(int i = MAX_ORDER-1;i>=0;i--){
			// it the remainder is zero, 
			// return 1 because all pages are successfully allocated
			if(remainder ==0){
				return 1;
			}
